...
- **新機／檢測新機**：方案名稱不影響比對；只要 EPB SAcare 與保險端序號一致即視為相符。EPB 無序號時才另頁按品類數量核對。
- **訂金／尾款**：訂金 (`G`) 依下訂月份與保險登記比對。同月 G/H 若同員工且品項、數量完全一致，會合併為一筆並用尾款主機序號配回訂金；跨月 H 若能追溯到前月相同序號的訂金，視為重複而不再計算。無法建立訂金關聯的尾款仍照常核對，避免漏掉真異常。
...
另含：**品類數量比較**、**逐日總量比較**、**訂金已納入比對明細**等健檢表。
